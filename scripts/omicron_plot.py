...
df = df[df.reason == 'N']
#%%
daily_omicrons = df.resample('D',on='date')['lineage'].apply(lambda x: (x.isin(['BA.1','BA.1.1','BA.2','BA.3','B.1.1.529', "B.1.617.2"])).sum())
...
plot_df.to_csv('plots/omicron_pivot.csv', encoding='utf-8')
...
def plot_omicron_share(df, reason, scale):
    ...
    daily_omicrons = df_reason.resample("D", on="date")["lineage"].apply(
        lambda x: (x.isin(['BA.1','BA.1.1','BA.2','BA.3','B.1.1.529', "B.1.617.2"]).sum())
    )
